<template>
  <div :style="style" :class="isActive ? 'active' : 'inactive'" class="vdr" @contextmenu.prevent="e => $emit('contextmenu', e)" @mousedown.stop.prevent="bodyDown($event)" @touchstart.stop.prevent="bodyDown($event)">
    <slot />
    <div v-for="(stick, index) in sticks" :key="index" :class="['vdr-stick-' + stick, isResizable ? '' : 'not-resizable']" :style="vdrStick(stick)" class="vdr-stick" @mousedown.stop.prevent="stickDown(stick, $event)" @touchstart.stop.prevent="stickDown(stick, $event)" />
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      bodyMove(ev) {
        ...
        const delta = {
          x: (this.axis !== 'y' && this.axis !== 'none' ? stickStartPos.mouseX - ev.pageX : 0) / this.parentScaleX,
          y: (this.axis !== 'x' && this.axis !== 'none' ? stickStartPos.mouseY - ev.pageY : 0) / this.parentScaleY
        };
        ...
        this.rawTop = Math.min(Math.max(stickStartPos.top - delta.y, this.miny), this.maxy);
        this.rawBottom = stickStartPos.bottom - this.rawTop + stickStartPos.top;
        this.rawLeft = Math.min(Math.max(stickStartPos.left - delta.x, this.minx), this.maxx);
        this.rawRight = stickStartPos.right - this.rawLeft + stickStartPos.left;

        // this.$emit('dragging', this.rect);
        let left = this.rawLeft;
        let top = this.rawTop;

        if(this.limits.minLeft != null && this.limits.maxLeft != null) {
          left = Math.min(Math.max(this.limits.minLeft, this.rawLeft), this.limits.maxLeft);
        }
        if(this.limits.minTop != null && this.limits.maxTop != null) {
          top = Math.min(Math.max(this.limits.minTop, this.rawTop), this.limits.maxTop);
        }
        this.$emit('dragging', { ...this.rect, left, top }); // fix:修复移动过程中不是left,top不是最终值的问题
      },

      bodyUp() {
        this.bodyDrag = false;
        // this.$emit('dragging', this.rect); // 修复释放鼠标后执行拖拽的问题
      ...
      stickDown: function(stick, ev) {
        // if(!this.isResizable || !this.active) { //fix:解决点击窗口意外的地方不能拖拽的问题
        if(!this.isResizable) {
          return;
        }
      ...
      stickMove(ev) {
        ...
        switch(this.currentStick[1]) {
          case 'r':
            this.rawRight = stickStartPos.right + delta.x;
            // this.rawRight = Math.min(stickStartPos.right + delta.x, this.left + this.maxw);
            break;
          case 'l':
            this.rawLeft = stickStartPos.left - delta.x;
            // this.rawLeft = Math.min(Math.max(stickStartPos.left - delta.x, this.minx, stickStartPos.right - this.maxw), this.maxx);
            break;
          default:
        }
